...
import { DefaultWebGLContext } from '../context/webgl/default/DefaultWebGLContext';

export class WebGLCanvas extends Canvas {
    constructor(canvasOptions, contextOptions) {
        super(canvasOptions);

        this.context = new DefaultWebGLContext(this.element.getContext('webgl', contextOptions) || this.element.getContext('experimental-webgl', contextOptions));
        if (!this.context) {
